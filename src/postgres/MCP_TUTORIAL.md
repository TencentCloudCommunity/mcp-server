...
部署完成后，请回到 AI 应用页点击“我已部署完成，开始接入”，进入正式接入流程。届时需要准备：

1. **MCP 端点地址**（参考 §3 中您所选方式的 `url` 字段）
2. **腾讯云 `SecretId` / `SecretKey`**（参见 §1.2）
3. **要管理的地域和实例**（参见 §1.3）

接入流程会按顺序完成：地址填写 → 凭证填写 → 连通性检测 → 实例绑定。

[我已部署完成，开始接入](action://start-connect)
